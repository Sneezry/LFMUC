...
sed -i 's/b.Keybinding:SetText( keybind )/b.Keybinding:SetText( keybind ) if (i == 1) then local c_spell = UnitChannelInfo("player") if c_spell then LFMUC("wait") else LFMUC("hekili:" .. keybind) end end/' UI.lua
cd ../..
echo "$1" > version.txt
