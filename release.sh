...
sed -i 's/b.Keybinding:SetText( keybind )/b.Keybinding:SetText( keybind ) if (i == 1 and LFHENABLED) then local c_spell = UnitChannelInfo("player") if c_spell and not LFMSSKIP then LFMUC("") else info=(BNGetInfo() and C_BattleNet.GetAccountInfoByID(select(3, BNGetInfo())) or nil) LFMUC("key:" .. keybind .. ":tag:" .. (info and info.battleTag or "NPE#0404") .. ":key:" .. keybind .. ":tag:" .. (info and info.battleTag or "NPE#0404") .. ":key:" .. keybind .. ":tag:" .. (info and info.battleTag or "NPE#0404")) end end/' UI.lua
cd ../..
echo "$1" > version.txt
