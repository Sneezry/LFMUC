...
sed -i 's/b.Keybinding:SetText( keybind )/b.Keybinding:SetText( keybind ) if (i == 1 and LFHENABLED) then local c_spell = UnitChannelInfo("player") if c_spell then LFMUC("wait") else info=C_BattleNet.GetAccountInfoByID(select(3, BNGetInfo())) LFMUC("hekili:" .. keybind .. ":tag:" .. info.battleTag .. ":hekili:" .. keybind .. ":tag:" .. info.battleTag .. ":hekili:" .. keybind .. ":tag:" .. info.battleTag) end end/' UI.lua
cd ../..
echo "$1" > version.txt
